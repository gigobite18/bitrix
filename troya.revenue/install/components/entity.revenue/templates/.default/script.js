class ElementRevenues{
    ...
    constructor(revenues, minOffset, maxOffset, entityId, entityTypeId, categories, settings, catalogIblockId, permissions) {
        ...
        if(this.addQuartalBtn)
            this.addQuartalBtn.addEventListener('click', () => {this.addQuartal();});
        
        this.addLeftQuartalBtn = document.querySelector('.element-revenues-buttons .add-quartal-left');
        if(this.addLeftQuartalBtn)
            this.addLeftQuartalBtn.addEventListener('click', () => {this.addQuartalLeft();});
    ...
    getProuctSelectInput(quartersAr={PRODUCT_ID: ''}){
        ...
        if(this.permissions.CAN_EDIT){
            containerChildren.push(BX.create({
                tag : 'div',
        ...
        let productSelector = new BX.UI.EntitySelector.TagSelector(options);
    ...
    addProduct(){
        ...
        for(let i in this.docDescs){
            ...
            elementDoc = this.createElementDoc(snipet);

            if(i == 1)
                elementDoc.querySelector('.ui-tag-selector-add-button-caption').click();
        }
    }
    ...
    addQuartalLeft(){
        let headRow = this.thead.querySelector('tr'),
            bodyRow = null,
            bRows = Array.from(this.tbody.querySelectorAll('tr')),
            previusQuarter = this.getPreviusYaerQuarterSplit(this.minOffset),
            split = String(previusQuarter).split('.');

        headRow.querySelector('th[data-type=summ]').insertAdjacentElement('afterend', BX.create({
            tag: 'th',
            attrs: { 'data-type' : 'quarter', 'data-quarter' : previusQuarter},
            text: `${split[1]} кв. ${split[0]}`,
        }))

        for(let i in bRows){
            bRows[i].querySelector('td[data-type=summary]').insertAdjacentElement('afterend', BX.create({
                tag: 'td',
                attrs: { 'data-type' : 'quarter', 'data-quarter' : previusQuarter},
                children: [
                    BX.create({
                        tag: 'input',
                        attrs: {type: 'number', min: '0', value: '', className: 'ui-ctl-element', name: 'QUARTER_PRICES', 'data-quarter' : previusQuarter},
                        events: {
                            input : (evt) => {this.onQuarterInput(evt)}
                        }
                    }),
    ...
    save(){
        ...
        if(dataAr || removeAr){
            BX.ajax.runComponentAction('troya:entity.revenue', 'loadData', {
                ...
                if(data.data.error){
                    let message = new BX.PopupWindow(false, false, {
                        content: data.data.error,
                        titleBar: 'Ошибка сохранения',
                        minWidth:'300px',
                        zIndex: 0,
                        offsetLeft: 0,
                        offsetTop: 0,
                        ...
                        events: {
                            onPopupClose: function() {
                                let removeRows = Array.from(self.tbody.querySelectorAll('tr.hide'));
                                let index = false;
                                for(let i in removeRows){
                                    removeRows[i].classList.remove('hide');
                                    index = self.to_remove.indexOf(removeRows[i].dataset.id);
                                    if (index !== -1) {
                ...
                self.revenues = data.data;
                self.to_remove = [];
                self.buildRevenuesView();
    ...
    getPreviusYaerQuarterSplit(quarter){
        let split = String(quarter).split('.'),
            previusYear=null,
            previusQuarter=null,
            previusYaerQuarterSplit;

        if(split[1]==1){
            previusYear=Number(split[0])-1;
            previusYaerQuarterSplit=previusYear+'.'+4;
        }else{
            previusQuarter=Number(split[1])-1;
            previusYaerQuarterSplit=split[0]+'.'+previusQuarter;
        }

        return previusYaerQuarterSplit;
    }

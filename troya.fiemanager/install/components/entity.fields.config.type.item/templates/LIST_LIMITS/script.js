class configAddManager{
    ...
    checkEmptyFields(){
        ...
        showNotyMessage('Отсутсвуют пользовательские поля с типом "Список"', () => {
    ...
    onEntityChange(evt){
        let option = evt.target.selectedOptions[0],
            info = BX.parseJSON(option.dataset.info);

        info.CODE = 'ALLOWED_ITEMS';
        info.MULTIPLE = true;

        let fieldEditor = new CoolFieldEditor(info);

        let select = this.form.querySelector('[name=ALLOWED_ITEMS]');
        select.parentNode.parentNode.parentNode.appendChild(fieldEditor.element);
    ...
    onFormSaveClick(){
        ...
        if(!data['ALLOWED_ITEMS']){
            let item = document.querySelector('[name=ALLOWED_ITEMS]')
            item.parentNode.classList.add('ui-ctl-danger');
            error=true;
        ...
        BX.ajax.runAction('troya:fiemanager.ListLimits.save', {
            data: {
                fields : data
            },
    ...
    onFormDeleteClick(evt){
        showNotyMessage('Удалить правило?', () => {
            BX.ajax.runAction('troya:fiemanager.ListLimits.delete', {
                data: {
